...
> _Grouchy grannies are on the villager team but the villagers wish they were not. Every day a grouchy granny picks a player to remove from the game for the rest of the day and night. They are not allowed to talk but can still watch what is happening. The Granny can use the \`/mute\` command only during the day. When a player is muted they will not be able to use commands so they lose their night ability if they have one after they return they will not be able to be muted again. The players that are muted will go to the out cast channel. They will be able to talk there and leave messages for others to read when they get muted. After they are unmuted they will leave the out cast channel._
______
`;
const howToPlay5 = `
