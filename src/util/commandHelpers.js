...
const powerUpMessages = new Map([
  ...
  [
    ...
    "You have Predator vision which allows you to look at a player and find their true character. Use command `/predator_vision` to see the player's true role.",
  ],
  [
    PowerUpNames.STEAL,
  ...
  try {
    ...
    switch (user.info.assigned_identity) {
      ...
      case characters.WITCH:
        await member.send(
          `You are a **Witch**.\n${voteText}\nYou are on the werewolf team, but you don't know which players are the werewolves.\nYou can curse a player in the game with the \`/curse\` command.\nWhen you are hanged by the villagers, the players that are cursed will die.\nWerewolves are not affected by the curse.\nIf the werewolves kill you, your curse does nothing.`
        );
        break;
      case characters.VAMPIRE:
        await member.send(
          `You are a **Vampire King**\n${voteText}\nVampires are on their own team. Bite other players to turn them into a vampire by using the command \`/vampire_bite\`.\nIf you try to bite a werewolf, you die.\nIf you bite someone the same night as the werewolves kill that victim, you will also die.\nYou have to bite the victim **2 times** before they turn into a vampire.`
        );
        break;
      case characters.DOPPELGANGER:
        await member.send(
          `You are a **Doppelganger**\n${voteText}\nYou don't know what team you are on yet. Use the \`/copy\` command to copy another player's character. If that player has a chat, you will join them. If you do not choose by the next day, the bot will choose for you.`
        );
        break;
      case characters.GROUCHY_GRANNY:
        await member.send(
          `You are a **Grouchy Granny**\n${voteText}\nYou can mute someone out of town square using the \`/mute\` command for the rest of the day and night. They will come back tomorrow, but while they are gone, they will be able to leave messages in the outcast channel. This will not allow them to use their night power. You will not be able to mute the same player for the rest of the game.`
        );
        break;
